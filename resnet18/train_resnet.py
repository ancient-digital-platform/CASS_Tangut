...
def train(data_dir, dataset_name):
    ...
    # 模型初始化
    model = ResNet18Model(num_classes=num_classes, pretrained=True).to(device)
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        if val_acc > best_acc:
            ...
            # 保存模型权重
            torch.save(model.state_dict(), f"{output_dir}/best_resnet18.pth")
        ...
        # 保存最新模型
        torch.save(model.state_dict(), f"{output_dir}/latest_resnet18.pth")
    ...
    # 加载最佳模型
    if os.path.exists(f"{output_dir}/best_resnet18.pth"):
        model.load_state_dict(torch.load(f"{output_dir}/best_resnet18.pth", map_location=device))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='训练ResNet18模型')
