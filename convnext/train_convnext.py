...
def train(data_dir, dataset_name):
    ...
    # 模型初始化
    model = ConvNeXtModel(num_classes=num_classes, pretrained=True).to(device)
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        if val_acc > best_acc:
            ...
            # 保存模型权重
            torch.save(model.state_dict(), f"{output_dir}/best_convnext.pth")
        ...
        # 保存最新模型
        torch.save(model.state_dict(), f"{output_dir}/latest_convnext.pth")
    ...
    # 加载最佳模型
    if os.path.exists(f"{output_dir}/best_convnext.pth"):
        model.load_state_dict(torch.load(f"{output_dir}/best_convnext.pth", map_location=device))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='训练ConvNeXt模型')
