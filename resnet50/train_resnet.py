...
#         ap = average_precision_score(labels_one_hot[:, class_idx], probs[:, class_idx])
#         aps.append(ap)

#     # 返回mAP
#     return np.nanmean(aps) * 100  # 处理可能出现的NaN值
...
def train():
    ...
    # 模型初始化
    model = ResNet50CatDog(pretrained=True).to(device)
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        if val_acc > best_acc:
            best_acc = val_acc
            os.makedirs("outputs-generated-enhanced-6145", exist_ok=True)

            # 保存模型权重
            torch.save(model.state_dict(), "outputs-generated-enhanced-6145/best_resnet50.pth")
            ...
            torch.save(val_metrics, "outputs-generated-enhanced-6145/best_val_metrics.pth")

            print(f"🏆 新最优模型已保存! 验证Top1准确率: {val_acc:.2f}%")

        # 保存最新模型
        torch.save(model.state_dict(), "outputs-generated-enhanced-6145/latest_resnet50.pth")
    ...
    # 加载最佳模型
    if os.path.exists("outputs-generated-enhanced-6145/best_resnet50.pth"):
        model.load_state_dict(torch.load("outputs-generated-enhanced-6145/best_resnet50.pth", map_location=device))
    ...
    # 保存测试结果
    with open("outputs-generated-enhanced-6145/test_results.txt", "w") as f:
        f.write(f"=== 模型信息 ===\n")
    ...
    print(f"📁 结果保存至: outputs-generated-enhanced-6145/test_results.txt")
    print(f"{'=' * 60}")
