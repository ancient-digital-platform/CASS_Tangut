...
def test(data_dir, dataset_name, model_path=None, output_dir=None):
    ...
    if model_path is None:
        model_path = f"{output_dir}/best_efficientnet.pth"

    print(f"\n{'=' * 60}")
    print("🧪 开始测试EfficientNet...")
    ...
    # 模型初始化
    print("📥 加载模型...")
    model = EfficientNetModel(num_classes=num_classes, pretrained=False).to(device)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='测试EfficientNet模型')
